...
学习被拆成两个仓库，**并行推进、节奏不同、互相喂养**：

```
quant_learn（本仓库）：实践与工程        节奏：按周计
  数据 → 回测 → 因子 → 风控 → 实盘
        ↑                    ↓
     被卡住时反查         提供动手场景
        ↓                    ↑
math_learn：英文原版数学地基            节奏：按月计
  证明 → 线代 → 概率 → 统计 → 优化
```

| | quant_learn | [math_learn](https://github.com/survivorff/math_learn) |
|---|---|---|
| 目标 | 跑出一套能用的交易系统 | 数学地基 + 英文技术阅读能力 |
| 时间尺度 | 12 个月出实盘 | 路径 A 9-12 个月，完整 2-3 年 |
| 硬指标 | 在线率、实盘-回测偏差、成本占比 | 做题数、独立完成率、阅读速度 |
| 失败模式 | 过拟合、爆仓、系统 bug | 只读不做题、卡住就停 |

### 关键原则：不要串行

❌ **错的做法**：先花 1 年学完数学，再开始做量化
理由：数学会忘掉大半，而且这一年没有任何可展示的产出。

✅ **对的做法**：两条线并行。这里被数学卡住 → 去 math_learn 补那一块 → 回来用掉它。
理由：带着具体问题去学，效率是漫无目的看书的十倍，而且立刻用掉就记住了。

**接口**：[math_learn 的"数学 → 量化对照表"](https://github.com/survivorff/math_learn/blob/main/ROADMAP.md) 是双向导航表。
...
### 学什么
- [`00-foundations`](./00-foundations/)：量化专用的结论与陷阱（对数收益、√N 缩放的失效条件、多重检验、协方差三陷阱、pandas 未来函数）
...
**并行**（[math_learn](https://github.com/survivorff/math_learn)）：00 证明与数学语言 → 01 线性代数。
这两块是后面所有数学的地基，且线代对量化的用处最直接（协方差、PCA、组合优化）。
